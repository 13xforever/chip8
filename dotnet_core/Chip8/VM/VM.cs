...
public class VM
{
    public readonly Registers Registers = new();
    public readonly Stack<ushort> Stack = new(32);
    ...
    {
        FontLoader.FromResource("hex_font.png", new(Ram, 0, 0x80));
        this.name = name;
        Console.Title += ": " + name;
    }
        ...
        switch (i1 & 0xf0)
        {
            ...
            case 0xf0:
            {
                switch (i2)
                {
                    ...
                    case 0x55:
                    {
                        for (ushort i = Registers.IR, ri = 0;
                             i < Math.Min(Registers.IR + GetX(ref i1), Ram.Length);
                             i++, ri++)
